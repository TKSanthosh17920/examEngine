...
//   }, []);
    ...
    useEffect(() => {
        const checkImage = async () => {
            ...
            try {
                const response = await axios.get(`http://localhost:5000/fetch-photo/${candidateInfo.user}`, {
        ...
        const checkSImage = async () => {
            console.log('useeeeeee',candidateInfo.user);
            try {
                const response = await axios.get(`http://localhost:5000/fetch-sign/${candidateInfo.user}`, {
                    responseType: 'blob' // important to get the image as a blob
                });
                setImageExists(true);
                const imageURL = URL.createObjectURL(response.data);
    ...
    //         try {
    //             const response = await fetch(`http://localhost:5000/questions/${candidateInfo.question_paper_no}/${candidateInfo.encryptKey}`);
    ...
    useEffect(() => {
        ...
        const fetchInitialData = async () => {
            try {
                ...
                const response = await fetch(`http://localhost:5000/initialAnswers/${candidateInfo.question_paper_no}`);
                const data = await response.json();
    ...
    const handleNextQuestion = async () => {
        ...
        if (entries.length > 0) {
            if (answers.hasOwnProperty(currentqp)) {
                ...
                await insertResponse({ 0: currentqp, 1: answers[currentqp], 2:questions[currentQuestionIndex].id });

            } else {
                await insertResponse({ 0: currentqp, 1: "NULL", 2:questions[currentQuestionIndex].id });
            }
        } else {
                            ...
                            <div className='row candidate-segment'>
                                ...
                                {imageExists ? (
                                    <>
                                        ...
                                        <img src={imageSUrl} alt="Signature" className='candimg'/>
                                    </>
                                ) : (
                                    <>
                                ...
                                <div className='col-md-7'>
                                    <p className='context-label'>Membership Number<br /><span className='context'>{candidateInfo.user}</span></p>
                                    <p className='context-label'>Candidate Name<br /><span className='context'>{candidateInfo.candidate_name}</span></p>
                                </div>
                            </div>
                            <NumberPalette
                                ...
                                <div className='row'>
                                    ...
                                    <div className='col-md-4'>
                                        {enabletag===0 ? (
                                            ...
                                            <button className='tag-button' onClick={handleDeTagQuestion}>De-Tag Question</button>
                                        )}
                                ...
                                <div className='col-md-8'>
                                    <table className='infodetails'>
                                        <tbody>
                                            ...
                                            <tr>
                                                <th>Examination</th><td>:</td><td>{candidateInfo.exam_name}</td>
                                            </tr>
                                            <tr>
                                            ...
                                            <tr>
                                                <th>Score</th><td>:</td><td>{acScore} out of {totalScore}</td>
                                            </tr>
