...
const SecureBrowserEnableDisable = () => {
  ...
  useEffect(() => {
    try {
      ...
      console.error(
        "Error while fetching variable value from exam_settings - secure_browser"
      );
    }
  }, []);
    ...
    try{
        ...
        if(res.status == 200){
            if(res.data.returnValue == "Enabled")
                {setMessage("Secure Browser enabled successfully")}
            else{
                setMessage("Secure Browser disabled successfully")
            }
        }else{
...
export default SecureBrowserEnableDisable;
